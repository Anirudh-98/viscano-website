...
export default function IntroAnimation() {
    ...
    // Morphs as the section scrolls into the viewport (centered at 0.5)
    const morphProgress = useTransform(scrollYProgress, [0.3, 0.5], [0, 1]);
    const smoothMorph = useSpring(morphProgress, { stiffness: 40, damping: 20 });

    // 2. Scroll Rotation (Shuffling): Starts after morph
    const scrollRotate = useTransform(scrollYProgress, [0.5, 0.8], [0, 360]);
    ...
    return (
        <div ref={containerRef} className="relative w-full bg-[#0A0A0A]">
            {/* Container */}
            <div ref={stickyRef} className="relative h-screen w-full flex flex-col items-center justify-center perspective-1000 overflow-hidden">
                ...
                >
                    ...
                    <p className="text-sm md:text-base text-white/60 max-w-lg leading-relaxed">
                        From identity to interface, we craft every detail with intention. <br className="hidden md:block" />
                        Viscano is where strategy, design, and development come together under one roof.
                    </p>
                </motion.div>
